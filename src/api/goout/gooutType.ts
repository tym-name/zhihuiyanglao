...
export interface GoOutInfo {
  // 老人ID（必填）
  elderlyId?: number;
  // 外出开始时间（必填，格式：YYYY-MM-DD）
  startTime?: string;
  // 外出结束时间（必填，格式：YYYY-MM-DD）
  endTime?: string;
  // 陪同人员手机号（必填）
  mobile?: string;
  // 陪同人员地址（必填）
  ...
  // 以下是你原代码中存在但本次数据未传的字段，补充为可选/默认值
  id?: number; // 主键ID，新增时不传，后端生成
  companyId?: number; // 公司/机构ID，可根据业务补充
  state?: number; // 登记状态，默认0（未审核/待处理）
  addTime?: string; // 添加时间，后端自动生成
  addAccountId?: number; // 操作人ID，可从登录信息中获取
  elderlyName?: string; // 老人姓名，前端展示用，非提交必传
}
