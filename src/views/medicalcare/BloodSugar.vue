<template>
    <div>
        <Table ref="tableRef" :columns="columns" :fetch-data="getBloodSugarList" :init-params="searchForm"
...
const delOnlyOne = (id: number) => {
    ...
    ElMessageBox.confirm(
        ...
        .then(async () => {
            await delBloodSugarById(id); // 先执行删除接口
            ElMessage({
                type: 'success',
                message: '删除成功',
            })
...
const selectionData = ref<bloodSugarListItem[]>([])
const ids = ref<number[]>([])
const handleSelectionChange = (rows: bloodSugarListItem[]) => {
...
const delAll = async () => {
    ElMessageBox.confirm(
        ...
        .then(async () => {
            let res = await delBloodSugarAllByIds(ids.value)
            console.log(res);
            if (res.code === 10000) {
                ElMessage({
                    type: 'success',
...
//查询
const searchForm = ref<bloodSugarListParams>({
    name: '',
    begId: null,
})
//搜索
