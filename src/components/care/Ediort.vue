    ...
    <div>
        <div style="border: 1px solid #ccc">
            <Toolbar style="border-bottom: 1px solid #ccc" :editor="editorRef" :default-config="toolbarConfig"
                :mode="mode" />
            <Editor :style="style" v-model="valueHtml" :default-config="editorConfig" :mode="mode"
...
import { Editor, Toolbar, type InsertFnType } from '@wangeditor/editor-for-vue'; // 编辑器实例，必须用 shallowRef
...
const editorConfig = {
    ...
    MENU_CONF: {
        uploadImage: {
            ...
            // 请求头 获取token令牌
            headers(): Record<string, string> { // 改为函数形式，确保每次请求都能获取最新 Token
                return {
